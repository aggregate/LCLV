/* LCLVdigital.ino
   ...
   Digital LCLV control example using the
...
#define	AN    0	  // use digital control
...
void loop() {
  ...
  Serial.println("Minimum opacity:");
  lclv.set(0);
  t = micros() + WAITT;
  Serial.println("lclv.set(0)");
  while (micros() < t) lclv.poll(); // polling makes AC waveform
  
  Serial.println("Maximum opacity:");
  lclv.set(255);
  t = micros() + WAITT;
  Serial.println("lclv.set(255)");
  while (micros() < t) lclv.poll(); // polling makes AC waveform
